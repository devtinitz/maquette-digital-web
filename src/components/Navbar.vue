...
import Logo from './Logo.vue' // Utilisation du nouveau composant Logo
...
<template>
  <nav class="fixed top-0 left-0 w-full z-[100] px-6 py-4">
    ...
    >
      <!-- Logo avec le nouveau composant -->
      <a href="/" class="flex items-center gap-2 group transform hover:scale-105 transition-transform duration-300">
        <Logo />
      </a>
      ...
      <div class="hidden md:flex items-center gap-6">
        <button class="text-[10px] font-black uppercase tracking-widest text-tinitz-navy hover:text-tinitz-red transition-colors">
          Connexion
        </button>
    ...
    >
      <div v-if="isMenuOpen" class="absolute top-24 left-6 right-6 bg-white rounded-3xl p-8 shadow-2xl md:hidden border border-slate-100">
        <div class="flex flex-col gap-6 text-center">
          ...
          <button class="bg-tinitz-red text-white w-full py-4 rounded-full font-black text-xs uppercase tracking-[0.2em]">Démo Gratuite</button>
        </div>
      </div>
    </Transition>
  </nav>
